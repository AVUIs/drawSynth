...
void ofApp::keyPressed(int key){
    ...
    if(key=='k'){
        ...
        if (curved) {
//            ofBackground(15);
            ofColor randomColor(ofRandom(255), ofRandom(255),ofRandom(255),255/curveVertices.size());
            selectedColor.clear();
...
//            double thisSine = wave + sineBank[i].sinewave(f0 + (i * f1))*sin(i)+ sineBank[i-1].square(f1);
            double thisSine = wave+(sineBank[i-1].sinewave(f0)*sin(i))*(sineBank[i].sinewave(f1)*cos(i));
        ...
        if (curveVertices.size()>0){
            wave *= curveVertices.size();
            ofNormalize(wave, 0, .5);
